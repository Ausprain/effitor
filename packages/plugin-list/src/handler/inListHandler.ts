...
const checkDeleteList = (ctx: Et.UpdatedContext) => {
  ...
  if (prevEl) {
    ...
    return ctx.commandManager
      ...
      .handleAndUpdate(cr.caretInStart(newP))
  }
}
  ...
  if (prevLi) {
    ...
    if (EtListElement.is(currListParent)) {
      ...
      return ctx.commandManager
        ...
        .handleAndUpdate(cr.caret(newP, 0))
    }
  }
  else {
...
export const inListHandler: Et.EffectHandlerWith<EtListItemElement, EtListItemElement> = {
  ...
  listItemMoveUp(ctx, { targetCaret: tc }) {
    ...
    if (prevEl) {
      ...
      return ctx.commonHandlers.moveNode(prevEl, cr.caretOutEndFuture(currLi))
    }
    // 无前兄弟li, 判断是否有前 list
    ...
    return ctx.commonHandlers.moveNode(
      currLi,
  ...
  listItemMoveDown(ctx, { targetCaret: tc }) {
    ...
    if (nextEl) {
      ...
      return ctx.commonHandlers.moveNode(nextEl, cr.caretOutStartFuture(currLi))
    }
    // 无后兄弟li, 判断是否有后 list
    const currList = currLi.parentElement as EtListElement | null
    if (!currList) {
