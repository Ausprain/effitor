...
export const insertCompositionText = platform.isSupportInsertFromComposition
  ...
  ? () => true
  : createInputEffectHandle((ctx, pl) => {
      if (!pl.data) {
        return true
      }
 ...
 */
export const insertFromComposition = createInputEffectHandle((ctx, { data, targetRange }) => {
  // FIXME 在 Safari 中, 输入法插入内容后, 若当前块节点为空, 会自动插入一个<br>, 该行为似乎无法阻止
  // 类似的, 在删除一个块节点的内容以致其无子节点时, 也会自动插入一个<br>
  if (!data) {
    return true
  }
  return insertTextAtCaret(ctx, data, targetRange.toTargetCaret())
})
