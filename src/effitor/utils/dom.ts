...
export const removeStatusClassOfEl = <E extends HTMLElement>(el: E): E => (el.classList.remove(Et.CssClass.Active, Et.CssClass.Selected), el)


export const zwsText = () => new Text(Et.HtmlChar.ZERO_WIDTH_SPACE)
...
export const cleanFragment = (f: DocumentFragment) => {
    ...
    traverseNode(f, (node) => {
        if (
            isElementNode(node) && node.elType === undefined && node.innerText === '' && !['BR', 'IMG'].includes(node.nodeName)  // node.nodeName !== 'BR'
